...
try:
    ...
    try:
        ...
        if (major, minor, patch) < (0, 8, 8):
            sys.stderr.write(
                'Warning: the Nativo library requires that your Python '
                '"requests" library be newer than version 0.8.8, but your '
                '"requests" library is version %s. Nativo will fall back to '
                'an alternate HTTP library so everything should work. We '
                'recommend upgrading your "requests" library. If you have any '
                'questions, please contact support@nativo.com. (HINT: running '
...
def new_default_http_client(*args, **kwargs):
    if urlfetch:
        ...
        impl = Urllib2Client
        warnings.warn(
            "Warning: the Nativo library is falling back to urllib2/urllib "
            "because requests is installed. "
...
class RequestsClient(HTTPClient):
        ...
        try:
            try:
                ...
                raise TypeError(
                    ...
                    '"requests" library is not compatible with Nativo\'s '
    ...
    def _handle_request_error(self, e):
        if isinstance(e, requests.exceptions.RequestException):
            msg = ("Unexpected error communicating with Nativo.  "
...
class UrlFetchClient(HTTPClient):
    ...
    def __init__(self, verify_ssl_certs=True, proxy=None, deadline=55):
        ...
        if proxy:
            raise ValueError(
                ...
                "Set Nativo.default_http_client to either RequestsClient, "
                " or Urllib2Client instance to use a proxy.")

        self._verify_ssl_certs = verify_ssl_certs
        # GAE requests time out after 60 seconds, so make sure to default
        # to 55 seconds to allow for a slow Nativo
    ...
    def _handle_request_error(self, e, url):
        if isinstance(e, urlfetch.InvalidURLError):
            msg = ("The Nativo library attempted to fetch an "
                   "invalid URL (%r). This is likely due to a bug "
                   "in the Nativo Python bindings. Please let us know "
                   "at support@nativo.com." % (url,))
        elif isinstance(e, urlfetch.DownloadError):
            msg = "There was a problem retrieving data from Nativo."
        elif isinstance(e, urlfetch.ResponseTooLargeError):
            msg = ("There was a problem receiving all of your data from "
                   "Nativo.  This is likely due to a bug in Nativo. "
                   "Please let us know at support@nativo.com.")
        else:
            msg = ("Unexpected error communicating with Nativo. If this "
                   "problem persists, let us know at support@nativo.com.")
...
class Urllib2Client(HTTPClient):
    ...
    def _handle_request_error(self, e):
        msg = ("Unexpected error communicating with Nativo. "
               "If this problem persists, let us know at support@nativo.com.")
        msg = textwrap.fill(msg) + "\n\n(Network error: " + str(e) + ")"
        raise error.APIConnectionError(msg)
